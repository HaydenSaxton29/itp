...
function draw() {
  ...
  drawKnob(670, 12, 40, filterFreq, 100, 10000, "FLT");
  textSize(10);
  textAlign(CENTER);
...
function mousePressed() {
  ...
  // Filter frequency knob
  if (dist(mouseX, mouseY, 670, 32) < 20) {
    filterKnobActive = true;
    return;
  }
...
function mouseDragged() {
  if (filterKnobActive) {
    ...
    filterFreq = constrain(filterFreq + delta * 50, 100, 10000);
    filter.frequency.value = filterFreq;
  } else if (reverbKnobActive) {
    let delta = -(mouseY - pmouseY);
